...
W3.2 不实现企业微信 OAuth 登录、扫码绑定、消息推送和定时自动同步；相关数据接口保留扩展位置，分别在 W3.3 及后续阶段实现。
...
- **DB-014 / W3.3**：外部身份登录、企微 OAuth userid/unionid 绑定、解绑和账号合并尚未实现；`passwordLoginEnabled=false` 的同步成员只能由管理员管理，不能登录。
- **DB-006 / W3.3**：企业微信消息发送、渠道启停、失败重试和回执尚未实现。
