...
> 2026-08-27 在进入 W3.4.3 前复核发现模块设置中的“线索池设置 / 线索库容设置 / 移入线索池原因设置”仍为占位，因此曾暂停 W3.4.3 并重新打开补漏 task **3.7**。3.7 随后完成并通过专项/全量验收；2026-08-28 客户域 4.1～4.7 继续全部完成，W3.4.0～W3.4.5 现已整体 `VERIFIED`。线索补漏基线见 [线索模块设置补漏源码与实施审计](./clue-module-settings-audit.md)。
...
- [x] 4.6 重建客户域 Vue 页面
  - [x] 对齐客户、联系人、客户公海三入口及公共列表底座；普通客户补齐批量转移、批量移入公海与 Cordys 移池原因/目标公海 Dialog。
  - [x] 对齐客户 360、联系人启停/深链、协作、关系、合并和公海详情/批量操作；公海详情改用 `/pool/account/get` 独立只读 360，并应用当前 Pool 隐藏字段。
  - [x] 客户公海补齐批量领取、批量分配、批量修改、批量删除与单条领取/分配/删除；领取/分配后只触发一次列表刷新，避免重复 `/pool/account/page`。
  - [x] `/customers?id=...`、`/customers/open-sea?poolId=...&id=...`、`/contacts?id=...` 与公海联系人深链均按 Cordys 打开对应普通/公海客户详情。
  - [x] Mobile 客户/联系人/客户公海三个真实入口继续可用；Browser Smoke 实际切换 390px 手机视口验证客户公海加载真实 Pool，不是静态空壳。
  - _需求：R2、R7～R9_

- [x] 4.7 完成客户域专项验收并本地提交
  - [x] 根级 `pnpm smoke` **220/220**，覆盖客户→联系人→协作→关系→公海→领取→合并、公海越权、联系人引用拒删、租户隔离和事务回滚；专项客户 **22/22**、联系人 **18/18**、深层规则 **30/30**、公海 **36/36**、模块设置 **25/25**、rules **114/114** 全绿。
  - [x] Browser：客户域页面 **21/21**（含 Mobile）、模块设置 **17/17**；验证移入公海配置请求不重复、公海隐藏字段、普通/公海深链及关键批量动作。
  - [x] 根级 typecheck、受影响文件 ESLint、production build 与 `git diff --check` 全绿；完成后本地提交。
  - _需求：R7～R9、R11、R12_
