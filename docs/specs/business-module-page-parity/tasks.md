...
- [ ] 1.4 实现模块表单与动态字段底座
  ...
  - [ ] 主记录与字段值使用同一事务；列表、详情、表单、导出不再读取 `customData`。公共事务接口和真实库回滚已验证，目标业务调用方待任务 1.7 切换。
  - [x] 添加普通值、Blob、唯一、批改、筛选和事务回滚规则测试。
  - 实施证据：[W3.4.0 模块表单与动态字段底座实施记录](./field-foundation-audit.md)。
  - _需求：R1、R2、R5、R7、R8_

- [ ] 1.5 实现用户视图直接模型与公共 Service
  - 完成新增、编辑、删除、固定、启停、拖拽排序和条件树序列化。
  - 按用户、组织和 `resourceType` 隔离；系统视图保持代码事实，不写入用户视图表。
  - 将现有线索、客户、联系人列表调用切换到新 Service，删除 SavedView 数据库访问。
  - _需求：R2、R5～R9_
