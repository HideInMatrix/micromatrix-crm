...
- 本单元只完成公共用户视图和当前图中五类资源调用；线索、客户、联系人主表仍处于 W3.4 直接模型迁移中间态，由任务 1.6～1.8 继续消除。
- `optionMap` 当前返回空对象；基于动态字段数据源的回显选项需要在任务 1.7 接入目标业务表单和值服务后补齐，不回读旧 SavedView 数据。
- 当前开发库不会提前应用破坏性迁移。待 1.6～1.9 全部调用方、Seed 和 Smoke 完成后再统一重建。
