...
1. **查看真实页面**：先在当前 Cordys 实例打开目标页面，记录模块开关、角色权限过滤后的可见入口、页面结构、字段、操作、状态和交互流程。
2. **跟踪页面接口**：根据页面实际发起的请求确认接口路径、参数、响应、错误语义和调用时序，不凭前端名称猜测后端能力。
3. **阅读后端实现**：沿接口依次检查 Controller、Service、Domain、DTO、Mapper XML，以及权限、数据范围、日志、通知、审批和定时任务等关联实现。
...
6. **逐页闭环**：公共依赖可用后，再按“页面 → 接口 → 后端语义 → NestJS/Vue 实现 → 测试”的闭环逐个对齐业务模块。
