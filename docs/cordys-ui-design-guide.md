...
第一层是业务动作层：左侧新建、导入、导出全部；右侧搜索、高级筛选、列设置、列表/看板模式、全屏、刷新。右侧工具按钮使用 16px 图标、32px 点击区和 8px 间距；使用 Element Plus `el-button` 组合时必须显式消除 `.el-button + .el-button` 的默认 `margin-left`，间距只允许由父级 `gap` 控制，避免同组按钮出现 8px/20px 混合间距。搜索动作集成到输入框后缀，不额外增加“搜索”按钮。

第二层是视图层：左侧系统视图、固定个人视图和“+ 新建视图”，右侧统一视图选择器。MicroMatrix 当前确认采用简化视图选择器：选择器左侧不再显示额外“视图”前缀，展开后只展示可选择的真实视图项，不显示“+ 新建视图 / 系统视图 / 个人视图 / 管理视图”等额外蓝色文案。该点为项目确认后的视觉覆盖规则，不再机械复刻 Cordys `CrmViewSelect` 的 header/action/group label。列设置不属于视图层。
