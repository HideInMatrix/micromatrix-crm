...
- `PRISMA8-003`：`IN_PROGRESS`。复核确认 `apps/api/src/prisma/generated/` 当前仍是 `@prisma/orm-postgres` 的 `contract.json / contract.d.ts` emit 目录，production runtime 仍手工加载 `contractJson` 并构造 `PostgresClient<Contract>`；本单元目标是验证并切换到官方 generated `prisma-client`，由 generator 正式输出 CommonJS（`moduleFormat = "cjs"`），同时保持单一 canonical database contract 和现有 migration ownership。当前执行 P0.5→P1.1 feasibility gate，production runtime 尚未切换。
