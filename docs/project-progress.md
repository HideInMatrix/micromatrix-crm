...
- `PRISMA8-003`：`VERIFIED / NO_GO_CLIENT_REVERT`。Prisma 8 官方文档确认 `contract.json / contract.d.ts` 是正式 generated contract artifact，PostgreSQL runtime 正式使用 `postgres<Contract>()`，model API 正式为 `.orm.public.*`；传统 `prisma-client` generator 的 `moduleFormat = "cjs"` 不属于 Prisma 8 contract runtime。项目保持当前 Prisma 8 architecture，不引入第二份 schema、不回退 traditional generated-client 模式。
