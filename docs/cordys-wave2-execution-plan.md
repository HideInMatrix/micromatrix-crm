...
- 跟进计划已在 W2.2 完成，消息设置底座已在 W2.3 完成，业务消息触发链路已在 W2.4 完成；全局搜索、国际化和 Agent 仍未纳入或未实现，不得用空壳入口伪造完成。
...
- W2.3 当时只建立消息设置底座；真实业务通知触发点随后由 W2.4 按 Cordys 事件逐项接入，不在 W2.3 臆造尚未存在的触发点。
...
- 从客户、联系人、协作团队、线索/资源池、商机、报价、审批完成通知和 `NoticeExpireJob` 逐层读取 Cordys Service、Listener、任务与公共接收人解析逻辑。
- 确认人工操作排除操作者、线索转客户/商机拆分事件、审批对象事件映射，以及到期任务按租户开关、提前天数和接收范围运行。
- 当前模型可准确表达 29 个新增业务事件；加上 W2.3 的 3 个跟进计划事件，共 32/35 个目录事件具备真实触发链路。

### 本阶段落地范围

- 新增统一业务消息分发层，集中处理接收人去重、操作者排除、租户/成员状态过滤和 best-effort 错误隔离。
- 接入客户、联系人、协作团队、线索/线索池、商机、报价和审批结果事件；自动回收与人工移池使用不同事件，回款审批不冒充发票审批。
- 新增配置接收范围解析，支持负责人、创建人占位符、指定成员、角色成员以及当前/上级部门负责人。
- 新增每天 08:00 的统一到期执行器，覆盖报价、合同和回款计划的即将到期/当天到期六个事件，并替换固定 3 天回款提醒任务。

### 验收结果

- Prisma validate、shared/API/Web typecheck、ESLint、API/Web build 全绿。
- 规则与公共底座单测 `27/27`；全链路 smoke `214/214`。Smoke 覆盖真实事件关闭/恢复、客户人工移入公海、线索人工移池与双事件转换、商机转移和合同审批结果；固定时钟测试覆盖到期 3/7 天、当天、关闭/空配置、执行中状态和足额回款过滤。
- 消息设置页面沿用 W2.3 已验收界面，本阶段未新增页面或公开 API；所有配置在 Smoke 结束前恢复为系统消息开启和默认提前 3 天，浏览器刷新复验没有新增 console error/warn。

### 明确保留的后置能力

- `CONTRACT_ARCHIVED`、`CONTRACT_VOID`、`INVOICE_APPROVAL` 不伪造触发点，仍为未接入的 3/35。
- 合同阶段/作废审计、发票审批、创建人审计、回款计划独立负责人、第三方渠道、公告和模板国际化缺口统一登记在 [Cordys 暂缓能力与数据模型缺口台账](./cordys-deferred-backlog.md)，后续阶段提交前必须复查状态。
...
- 已读取 Cordys 流程列表、新建/编辑/详情抽屉、基础信息、更多设置、流程设计器模型、前端 API，以及 `ApprovalFlowController/Service`、Domain、Mapper 与 1.7.0/1.7.2 数据库迁移。
- 已确认 Cordys 对每个租户/表单类型只保留一条未删除流程，采用软删除、独立启停和操作日志；节点配置更新创建新版本，实例绑定提交时的版本。
- 已确认目标表单类型为报价、合同、发票、订单；当前 MicroMatrix 的回款审批不能替代发票审批。
- 已确认独立“工作流”页面在 Cordys 当前源码中只是占位，不纳入实现。

### 已完成实施

- 将当前单页流程编辑改为流程列表与新增/编辑/详情抽屉。
- 补齐流程编号、表单类型、执行时机、描述、软删除、创建/更新审计和独立 CRUD 权限。
- 建立流程版本、版本化节点和节点连接底座，迁移现有线性配置，并保持既有审批实例可读可运行。
- 流程设计区使用 Vue Flow 实现受控线性图，前端画布状态必须转换为共享业务契约并接受服务端结构校验。
- 保留当前真实可用的新建/提交审批；发票业务链路、编辑/删除拦截和高级节点运行时不伪造完成。
- 所有后置数据模型缺口已登记在 [暂缓能力台账](./cordys-deferred-backlog.md)。

需求文档：[流程设置管理底座对齐需求](./specs/process-settings-parity/requirements.md)。

技术设计：[流程设置管理底座技术设计](./specs/process-settings-parity/design.md)。

实施任务：[W2.5 实施任务](./specs/process-settings-parity/tasks.md)。

### 验收结果

- Prisma 迁移已落地并在本地 PostgreSQL 成功执行；再次执行 `migrate deploy` 不重复创建版本、节点或权限关系。
- 流程配置 API、独立四类权限、操作日志、软删除、发票启用拦截、回款新提交退出和报价/合同/订单运行时版本绑定已接通。
- Web 已改为真实分页列表、三模式抽屉与 Vue Flow 受控线性设计器；更新/删除执行、高级设置和发票运行时均明确禁用或标记待接入。
- 规则与公共底座单测 `27/27`、API/Web 类型检查与构建通过；全链路 Smoke `219/219`，浏览器验证列表、抽屉、设计步骤、节点新增/编辑及控制台无新增 error/warn。
