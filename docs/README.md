...
- 全链路回归：`pnpm smoke`（当前 **225 条实际断言**）+ `pnpm smoke:wecom-sync`（23 条 W3.2 断言）+ `pnpm smoke:wecom-sso-message`（19 条 W3.3 断言）+ `pnpm --filter @micromatrix/api test:rules`（95 条规则与公共底座单测）
