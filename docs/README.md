...
- Wave 1 主线：[cordys-wave1-remainder-plan.md](./cordys-wave1-remainder-plan.md) 记录的 R1-R6 与 R7 多角色权限均已完成；后续先对齐公共基础能力，再按 [cordys-parity.md](./cordys-parity.md) 逐页对齐业务模块
- 全链路回归：`pnpm smoke`（当前 **186 条实际断言**）+ `pnpm --filter @micromatrix/api test:rules`（11 条规则单测）；W1.1-W1.7、R1-R6 与 R7 均已使用真实 Node 24.5.0 / pnpm 10.30.3 执行验收
