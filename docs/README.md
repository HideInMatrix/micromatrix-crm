...
| [cordys-graph-completion-plan.md](./cordys-graph-completion-plan.md) | 用户功能图收口计划；W3.4.0 task 1.1～1.9 已验收，下一步进入 W3.4.1 首页         |
...
- 当前主线：[cordys-graph-completion-plan.md](./cordys-graph-completion-plan.md)；W3.4.0 公共底座 1.1～1.9 已完成全量专项验收，下一步执行 W3.4.1 首页
- 全链路回归：`pnpm smoke`（当前 **219 条实际断言**）+ `pnpm smoke:wecom-sync`（23 条 W3.2 断言）+ `pnpm smoke:wecom-sso-message`（19 条 W3.3 断言）+ `pnpm --filter @micromatrix/api test:rules`（97 条规则与公共底座单测）
