...
- Wave 1 主线：[cordys-wave1-remainder-plan.md](./cordys-wave1-remainder-plan.md) 记录的 R1-R6 已全部完成；后续工作按 [cordys-parity.md](./cordys-parity.md) 中未完成模块重新立项
- 全链路回归：`pnpm smoke`（当前 **180 条实际断言**）+ `pnpm --filter @micromatrix/api test:rules`（7 条回收规则单测）；W1.1-W1.7 与 R1-R6 均已使用真实 Node 24.5.0 / pnpm 10.30.3 执行验收
