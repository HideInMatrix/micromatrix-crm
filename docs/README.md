...
| [cordys-graph-completion-plan.md](./cordys-graph-completion-plan.md) | 用户功能图收口计划；W3.4.2 线索域已关闭，当前执行 W3.4.3 task 4.1 客户域源码证据矩阵 |
...
- 当前主线：[cordys-graph-completion-plan.md](./cordys-graph-completion-plan.md)；W3.4.1 首页、W3.4.2 线索与线索池、企业设置 W3.4-S/前端目录收口以及插入的 **W3.4-D Docker 发布链路**均已完成，当前执行 **W3.4.3 客户、联系人和客户公海 task 4.1**
- 当前回归基线：`pnpm smoke`（**219/219**）+ 线索域连续生命周期（**17/17**）+ 普通线索 API（**18/18**）+ 三条转换（**21/21**）+ 多线索池（**32/32**）+ 线索 Browser（**20/20**）+ `pnpm smoke:enterprise-settings`（**23/23**）+ `pnpm smoke:w341-home`（17/17）+ `pnpm smoke:w341-home-browser`（12/12）+ `pnpm smoke:wecom-sync`（23/23）+ `pnpm smoke:wecom-sso-message`（19/19）+ `pnpm --filter @micromatrix/api test:rules`（**114/114**）
