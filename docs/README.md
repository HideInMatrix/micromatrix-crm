...
| [cordys-graph-completion-plan.md](./cordys-graph-completion-plan.md) | 用户功能图收口计划；W3.4.1 首页已验收，下一步进入 W3.4.2 线索与线索池        |
...
- 当前主线：[cordys-graph-completion-plan.md](./cordys-graph-completion-plan.md)；W3.4.1 首页已完成源码→API→数据→页面→自动化→浏览器闭环，下一步执行 W3.4.2 线索与线索池
- 全链路回归：`pnpm smoke`（当前 **219 条实际断言**）+ `pnpm smoke:w341-home`（17 条首页真实库断言）+ `pnpm smoke:w341-home-browser`（12 条首页 Chrome 交互断言）+ `pnpm smoke:wecom-sync`（23 条 W3.2 断言）+ `pnpm smoke:wecom-sso-message`（19 条 W3.3 断言）+ `pnpm --filter @micromatrix/api test:rules`（108 条规则与公共底座单测）
