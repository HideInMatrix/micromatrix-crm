...
| [cordys-graph-completion-plan.md](./cordys-graph-completion-plan.md) | 用户功能图收口计划；W3.4.4 Dashboard 资源/目录 API 已闭环，当前进入 task 5.4 |
...
- 当前主线：[cordys-graph-completion-plan.md](./cordys-graph-completion-plan.md)；W3.4.4 task 5.1～5.3 已完成 Dashboard 源码证据、命名空间释放与目录/资源 API，当前进入 **task 5.4：收藏与安全嵌入适配**。
- 当前回归基线：Dashboard API **31/31** + `pnpm smoke` **222/222** + rules **114/114** + 客户公海主体 **36/36** + 客户模块设置 API **25/25** + Browser **17/17** + 客户协作/关系/合并 **30/30** + 客户 API/360 **22/22** + 联系人 API **18/18** + 线索模块设置 API **22/22** + Browser **17/17** + 线索连续生命周期 **17/17** + 普通 API **18/18** + 转换 **21/21** + 多 Pool **32/32** + 线索 Browser **20/20**
