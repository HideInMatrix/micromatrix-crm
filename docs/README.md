...
| [cordys-wave2-execution-plan.md](./cordys-wave2-execution-plan.md) | Wave 2 执行记录；W2.1 顶部导航已验收，下一步为 W2.2 跟进计划 |
...
- 当前主线：[cordys-wave2-execution-plan.md](./cordys-wave2-execution-plan.md)；W2.1 顶部导航配置闭环已完成，下一步为 W2.2 跟进计划
- 全链路回归：`pnpm smoke`（当前 **190 条实际断言**）+ `pnpm --filter @micromatrix/api test:rules`（14 条规则与公共底座单测）；W2.1 已使用本地 PostgreSQL / Redis 真实链路执行验收
