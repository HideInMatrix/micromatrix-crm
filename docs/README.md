...
| [cordys-graph-completion-plan.md](./cordys-graph-completion-plan.md) | 用户功能图收口计划；当前 W3.3 已验收，下一步 W3.4 图中业务模块逐页复查          |
...
- 当前主线：[cordys-graph-completion-plan.md](./cordys-graph-completion-plan.md)；W3.3 企微统一登录与消息渠道已闭环，下一步为 W3.4 图中业务模块逐页复查
- 全链路回归：`pnpm smoke`（当前 **225 条实际断言**）+ `pnpm smoke:wecom-sync`（23 条 W3.2 断言）+ `pnpm smoke:wecom-sso-message`（16 条 W3.3 断言）+ `pnpm --filter @micromatrix/api test:rules`（65 条规则与公共底座单测）；W3.3 已使用本地 PostgreSQL、可控企微夹具与浏览器执行验收
