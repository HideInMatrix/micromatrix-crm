...
| [cordys-wave2-execution-plan.md](./cordys-wave2-execution-plan.md) | Wave 2 执行记录；W2.1 顶部导航与 W2.2 跟进计划均已验收 |
...
- 当前主线：[cordys-wave2-execution-plan.md](./cordys-wave2-execution-plan.md)；W2.1 顶部导航与 W2.2 跟进计划闭环已完成
- 全链路回归：`pnpm smoke`（当前 **199 条实际断言**）+ `pnpm --filter @micromatrix/api test:rules`（17 条规则与公共底座单测）；W2.2 已使用本地 PostgreSQL / Redis 真实链路执行验收
