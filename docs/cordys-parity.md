...
| -------------- | ----------------------------------------------------------------------------------------------------- | ----------------------- | ---- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 线索           | `clue/*`、`ClueService`、`PoolClueService`                                                            | `modules/leads`         | 🟡   | R1/R2/R4 功能链和 W3.4.0 直接模型底座均已通过，但按本文“✅=完成 Cordys 最终验收”的定义，W3.4.2 尚需重新闭环普通线索、转换、多池、分域配置 API 与页面，因此保持部分对齐 |
| 客户           | `customer/*`、`CustomerService`、`customerOverviewDrawer.vue`、Mobile `customer/index.vue/detail.vue` | `customers`             | 🟡   | R5/W2.2 功能链与 W3.4.0 Customer/Contact/Pool/Owner/Relation/Collaboration 直接模型底座已通过；W3.4.3 客户/联系人/公海/360 页面级最终闭环尚未完成                      |
| 联系人         | `CustomerContactController/Service`、`contact.vue`、`contactTable.vue`                                | `modules/contacts`      | 🟡   | R3/R5 已验证批改、导入导出、商机关联删除保护和直接字段模型，但联系人作为 W3.4.3 客户域的一部分仍待独立页面、客户 360 入口、字段/权限边界的最终源码级验收               |
