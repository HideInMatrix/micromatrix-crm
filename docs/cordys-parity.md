...
| 联系人         | `CustomerContactController/Service`、`contact.vue`、`contactTable.vue`                                | `modules/contacts`      | 🟡   | W3.4.3 task 4.1 已确认独立联系人和客户 360 共用表格、`customerId` 最终可空、无批量删除且商机关联由后端拒删；`/account/contact/*` API 与页面闭环待 task 4.3                               |
