...
> W3.4 用户确认功能图（首页、线索/线索池、客户/联系人/客户公海、仪表板）已于 2026-08-28 完成 W3.4.0～W3.4.5 最终验收并标记 `VERIFIED`。随后 W3.6 已继续关闭商机、产品/价格表、报价和合同主域；回款、发票、工商抬头、订单等后续交易域仍按下表 🟡 状态推进。
...
| 模块配置       | `SystemModuleService`、`views/system/module`                            | `modules/module-configs` + Pinia                                      | 🟡   | 页面已对齐主导航、顶部导航、配置行与模块动作；线索/客户、商机、产品/价格表、报价、合同表单与合同阶段已随对应 W3 单元逐项 REAL。合同卡中的回款计划/记录、工商抬头必填、发票当前明确 W3.6.4 deferred，不以旧模型伪装 REAL；其它模块继续按 DB-022 关闭 |
