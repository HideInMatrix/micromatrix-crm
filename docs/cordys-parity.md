...
| -------------- | ----------------------------------------------------------------------------------------------------- | ----------------------- | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 线索           | `clue/*`、`ClueService`、`PoolClueService`                                                            | `modules/leads`         | ✅   | R1/R2/R4 功能链保持通过；W3.4.0 已将 Clue/Field/Owner/Pool/Capacity 与既有调用方切到直接模型并完成 1.9 全量回归。W3.4.2 仍会按当前 Cordys 源码重新闭环线索/池页面与分域配置 API |
| 客户           | `customer/*`、`CustomerService`、`customerOverviewDrawer.vue`、Mobile `customer/index.vue/detail.vue` | `customers`             | ✅   | R5/W2.2 功能链保持通过；W3.4.0 已将 Customer/Contact/Pool/Owner/Relation/Collaboration 与交易链引用切到直接模型并完成 1.9 全量回归，W3.4.3 继续页面级复核                       |
| 联系人         | `CustomerContactController/Service`、`contact.vue`、`contactTable.vue`                                | `modules/contacts`      | ✅   | R3/R5 功能链保持通过；W3.4.0 已切 CustomerContact/Field/Blob 直接模型并验证批改、导入导出、商机关联删除保护和稳定 API alias                                                     |
...
| 模块配置       | `SystemModuleService`、`views/system/module`                          | `modules/module-configs` + Pinia                                      | 🟡   | 页面已按 Cordys `index.vue/configCard.vue` 对齐主导航、顶部导航、80px 配置行、模块动作与启停确认语义；租户级启停、排序、固定模块校验及动态菜单已完成。表单设置已接现有直接页面，池/库容/原因/状态流等专属设置按 DB-022 保持可见禁用，待对应业务模块直接 API/Page 完成后逐项启用 |
...
| 系统设置       | `OrganizationConfigService`、`IntegrationConfigService`               | `modules/settings` + `modules/enterprise-integrations`                | 🟡   | W3.1-W3.3 已完成企业信息/集成/API 分区、企微配置、组织同步、统一登录、企微消息和 Cordys 用户/OAuth 模型；其他企业平台仍待独立迁移                                                            |
