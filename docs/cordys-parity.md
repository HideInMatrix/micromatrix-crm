...
| 产品           | `product/*`、`price/*`、`ProductService`、`ProductPriceService`                                      | `modules/products`      | 🟡   | W3.6.1 已完成 Product/Price 直接模型、Field/Blob、PICTURE、SUB_PRODUCT、多级导入导出和页面；待 W3.6.2 报价直接模型完成后关闭“价格表被报价引用时禁止删除”的跨域保护                                      |
