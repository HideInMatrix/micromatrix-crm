...
| 站内通知       | `NotificationService`、`notice/*`、`MessageTask*`   | `modules/notifications` + `modules/message-settings` | 🟡   | W2.3 已完成五组 35 事件目录、租户/批量开关、8 个到期范围配置、独立权限/日志和事件分发门控；继续随业务模块绑定剩余触发点，邮件/第三方发送器后置        |
...
| 消息设置       | `views/system/message`、`MessageTaskController/Service`               | `modules/message-settings`                                            | 🟡   | W2.3 底座已完成；公告、邮件/第三方发送器和其余业务触发点后续独立对齐                                                                                                                         |
