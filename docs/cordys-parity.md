...
>
> 2026-09-01 当前执行指针：**W3.7-9.4A / DB-012 Condition / DEFAULT 图结构、条件 DTO 与 updateFields runtime**。DB-010、DB-011 均已 `VERIFIED`；DB-011 9.3A～E 已完成并在 63 migrations 下完成专项/回归封板。当前销售核心业务表 12/12 均为 ✅，剩余差异主要集中在 DB-012 高级节点运行时、协同深度、元数据/表单/搜索、审计/异步导出和少量平台/第三方能力。整体路线见 [project-progress.md](./project-progress.md)。
...
| 审批流         | `approval/*`                                        | `modules/approvals`                                  | 🟡   | W2.5 已完成基础版本图；W3.7 DB-010 已完成四业务通用资源快照/UPDATE 回放/DELETE 延迟执行；DB-011 9.3A～E 已完成 ApprovalRecord、task nodeRound/type/action、BEFORE/AFTER 嵌套加签、节点退回/ReturnBackRecord/round 重建、审批人任务撤回、requireComment 与 ApprovalInstanceAttachment。当前仅剩 DB-012 条件/动态审批人/字段权限/后置字段更新/Webhook 等高级节点能力。 |
