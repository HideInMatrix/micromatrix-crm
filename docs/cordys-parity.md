...
| 操作日志       | `BaseModuleLogService`、`SysOperationLogService`、`sys_operation_log_blob` | `modules/logs` + `BusinessChangeLogService`                           | 🚧   | LOG-001 已完成真实 IP/retention；LOG-002 已完成 Cordys 的轻量主表 + Blob、列表 + 详情分离，并作为 MicroMatrix 增强补齐租户 retention 网页策略/永久保留/立即清理；项目未上线，因此未保留旧 detail 兼容。该行继续为 🚧 仅因为业务对象覆盖和时间线 UI 仍待扩展。          |
...
| 异步导出中心   | `ExportTaskCenterService`                                                  | `ExportTask` + `ExportTasksService` + BullMQ + 独立 Export Worker     | ✅   | ASYNC-001 已完成 durable queue、独立 worker、10 个 PENDING 上限、同 module 防重复、取消/CAS、24h 保留、startup recovery 与真实 Redis/PostgreSQL restart Smoke；PostgreSQL task 为真相源，BullMQ job 仅携带 taskId                                                      |
