...
>
> 2026-09-02 当前执行指针：**W3.7-9.4C / DB-012 节点字段权限和审批详情真实约束**。DB-010、DB-011 均已 `VERIFIED`；DB-012 9.4A 已完成 Condition/DEFAULT 条件图，9.4B 已完成 empty/fallback/sameSubmitter、动态审批方向、多级审批人与 duplicate runtime，并在 65 migrations 下完成专项/回归封板。DB-012 整体保持 `IN_PROGRESS`。当前销售核心业务表 12/12 均为 ✅，剩余差异主要集中在 DB-012 字段权限/后置动作/Webhook/高级编辑器、协同深度、元数据/表单/搜索、审计/异步导出和少量平台/第三方能力。整体路线见 [project-progress.md](./project-progress.md)。
...
| 审批流         | `approval/*`                                        | `modules/approvals`                                  | 🟡   | W2.5 已完成基础版本图；DB-010 已完成四业务通用快照/UPDATE/DELETE 上下文；DB-011 9.3A～E 已完成高级 task/record/action/附件；DB-012 9.4A 已完成 Condition/DEFAULT 正式图与条件 runtime，9.4B 已完成 empty/fallback/sameSubmitter、BOTTOM_UP/TOP_DOWN、多级直属上级/部门负责人及 FIRST_ONLY/SEQUENTIAL_ALL/EACH duplicate runtime。剩余 9.4C～F：字段权限、后置字段更新、Webhook 和完整 Vue Flow 条件图。 |
