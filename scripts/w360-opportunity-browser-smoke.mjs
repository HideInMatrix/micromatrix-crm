...
async function closeDrawer(cdp, testId) {
  await cdp.evaluate(`document.querySelector('[data-testid=${JSON.stringify(testId)}] .el-drawer__close-btn')?.click()`)
  await sleep(150)
}
...
async function main() {
  ...
  if (!created.response.ok || !created.data?.id) throw new Error(`创建 Browser Smoke 商机失败: ${created.response.status} ${created.raw}`)
  ...
  try {
    ...
    await cdp.waitFor(`document.body.innerText.includes(${JSON.stringify(name)})`, 10000, 'Smoke 商机出现在列表')
    check('列表使用 Cordys POST /opportunity/page', cdp.requestCount('/api/opportunity/page', 'POST') >= 1)
    check('商机保存视图加载 /opportunity/view/list', cdp.requestCount('/api/opportunity/view/list', 'GET') >= 1)
    ...
    check('新建表单使用 Cordys 直接字段', ['商机名称','负责人','关联客户','联系人','商机金额','可能性','意向产品','结束时间'].every((item) => labels.includes(item)))
    ...
    check('批量转移/删除动作可见', await cdp.evaluate(`document.body.innerText.includes('批量转移') && document.body.innerText.includes('批量删除')`))
    ...
    await cdp.waitFor(`document.querySelectorAll('[data-testid="opportunity-board-column"]').length > 0`, 5000, '商机看板')
    check('商机看板使用可拖拽容器', await cdp.evaluate(`document.querySelectorAll('[data-testid="opportunity-board-column"]').length > 0`))
    ...
    await cdp.waitFor(`document.querySelector('[data-testid="opportunity-detail-drawer"]')?.innerText.includes(${JSON.stringify(name)}) === true`, 10000, '商机详情 Drawer')
    check('深链打开商机详情 Drawer', await cdp.evaluate(`document.querySelector('[data-testid="opportunity-detail-drawer"]')?.innerText.includes(${JSON.stringify(name)}) === true`))
    check('详情包含详情/联系人/跟进记录 Tab', await cdp.evaluate(`['详情','联系人','跟进记录'].every((x)=>document.querySelector('[data-testid="opportunity-detail-drawer"]')?.innerText.includes(x))`))

    await cdp.navigate('/system/modules')
    await cdp.waitFor(`document.querySelector('[data-module-config-key="opportunity"]') !== null`, 10000, '模块设置商机卡片')
    ...
    await cdp.waitFor(`document.querySelector('[data-testid="opportunity-stage-settings-drawer"]') !== null`, 5000, '商机阶段 Drawer')
    check('阶段 Drawer 包含回退设置与添加阶段', await cdp.evaluate(`document.querySelector('[data-testid="opportunity-stage-settings-drawer"]')?.innerText.includes('进行中允许回退') && document.querySelector('[data-testid="opportunity-stage-settings-drawer"]')?.innerText.includes('添加阶段')`))
    ...
    await cdp.waitFor(`document.querySelector('[data-testid="opportunity-close-rule-settings-drawer"]') !== null`, 5000, '商机关闭规则 Drawer')
    check('关闭规则 Drawer 为真实功能', await cdp.evaluate(`document.querySelector('[data-testid="opportunity-close-rule-settings-drawer"]')?.innerText.includes('添加规则') === true`))
    await closeDrawer(cdp, 'opportunity-close-rule-settings-drawer')

    await openOpportunityMore(cdp)
    check('商机失败原因菜单可点击', await clickVisibleMenuItem(cdp, '商机失败原因设置'))
    await cdp.waitFor(`document.querySelector('[data-testid="opportunity-failure-reason-settings-drawer"]') !== null`, 5000, '商机失败原因 Drawer')
    check('失败原因 Drawer 为真实功能', await cdp.evaluate(`document.querySelector('[data-testid="opportunity-failure-reason-settings-drawer"]')?.innerText.includes('最多 50 条') === true`))

    const api5xx = cdp.responses.filter((item) => item.status >= 500 && item.url.includes('/api/'))
    check('商机 Browser Smoke 无 API 5xx', api5xx.length === 0, api5xx.map((item) => `${item.status} ${item.url}`).join(', '))
    check('商机 Browser Smoke 无 Runtime exception', cdp.exceptions.length === 0, cdp.exceptions.join(', '))
  } finally {
    cdp.close()
  }
