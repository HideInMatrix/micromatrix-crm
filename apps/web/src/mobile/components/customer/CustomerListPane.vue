  ...
  <div class="h-full min-h-0 flex flex-col overflow-hidden bg-[#f7f8fa]">
    <div class="bg-white px-3 pt-2">
      ...
      <div v-if="auth.hasPerm('customer:create')" class="px-3 pb-2">
        <van-button type="primary" plain block size="small" @click="openCreate">新建客户</van-button>
      </div>
    </div>
      ...
      <van-list v-model:loading="loading" :finished="finished" finished-text="没有更多了" @load="loadMore">
        <van-cell-group v-for="item in items" :key="item.id" inset class="!mb-3">
          <van-cell
            ...
            @click="goDetail(item)"
          >
            <template #value><span class="text-xs">{{ item.ownerName ?? '-' }}</span></template>
          </van-cell>
          <van-cell>
            <template #title>
    ...
    <van-popup v-model:show="createShow" position="bottom" round :style="{ height: '85%' }">
      <div class="h-full flex flex-col">
        ...
        <div class="flex-1 overflow-auto"><MobileDynamicForm v-model="formModel" :fields="fields" /></div>
        <div class="p-4"><van-button type="primary" block :loading="saving" @click="handleCreate">保存</van-button></div>
      </div>
    </van-popup>
  </div>
</template>
