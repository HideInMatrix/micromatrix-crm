...
<template>
  <div class="h-full min-h-screen flex flex-col bg-[#f7f8fa]">
    ...
    <template v-else-if="customer">
          ...
          <div class="h-full overflow-auto pb-4">
            ...
            <template v-else-if="tab.name === 'record'">
              <div v-if="canWrite" class="px-4 pt-3">
                <van-button type="primary" plain block size="small" @click="followShow = true">新增跟进</van-button>
              </div>
            ...
            <template v-else-if="tab.name === 'header'">
              ...
              <van-cell-group v-for="item in ownerHistory" :key="item.id" inset class="!mt-3">
                <van-cell :title="item.ownerName ?? '未知负责人'" :value="item.departmentName ?? '-'" />
                <van-cell title="归属开始" :value="item.collectedAt ? new Date(item.collectedAt).toLocaleString() : '-'" />
            ...
            <template v-else-if="tab.name === 'relation'">
              <div v-if="canEditRelations" class="px-4 pt-3">
                <van-button type="primary" plain block size="small" @click="openRelationEditor()">添加客户关系</van-button>
              </div>
              ...
              <van-cell-group v-for="relation in relations" :key="relation.id" inset class="!mt-3">
                <van-cell :title="relation.customerName ?? '-'">
                  <template #label>{{ relation.relationType === 'GROUP' ? '集团客户' : '子公司' }}</template>
                  <template v-if="canEditRelations" #value>
                    <span class="text-[var(--van-primary-color)] mr-3" @click="openRelationEditor(relation)">编辑</span>
            ...
            <template v-else-if="tab.name === 'collaborator'">
              <div v-if="auth.hasPerm('customer:team')" class="px-4 pt-3">
                <van-button type="primary" plain block size="small" @click="openCollaboratorEditor()">添加协作人</van-button>
              </div>
              ...
              <van-cell-group v-for="member in collaborators" :key="member.id" inset class="!mt-3">
                <van-cell :title="member.userName" :label="member.role ?? '未设置角色'">
                  <template #value>
                    <van-tag plain :type="member.collaborationType === 'READ_ONLY' ? 'default' : 'primary'">
                ...
                <van-cell v-if="auth.hasPerm('customer:team')">
                  <template #value>
                    <span class="text-[var(--van-primary-color)] mr-4" @click="openCollaboratorEditor(member)">编辑</span>
      ...
      <div
        v-if="activeTab === 'info' && canMainAction && (auth.hasPerm('customer:update') || auth.hasPerm('customer:assign') || auth.hasPerm('customer:delete'))"
        class="shrink-0 bg-white border-t border-[#ebedf0] p-3 flex gap-3"
      >
        <van-button v-if="auth.hasPerm('customer:update')" block type="primary" @click="openEdit">编辑</van-button>
        <van-button
    ...
    <van-popup v-model:show="editShow" position="bottom" round :style="{ height: '88%' }">
      <div class="h-full flex flex-col">
        ...
        <div class="p-4"><van-button type="primary" block :loading="editSaving" @click="saveEdit">保存</van-button></div>
      </div>
    </van-popup>

    <van-action-sheet v-model:show="moreShow" title="更多操作">
      <div class="p-4 space-y-3">
        <van-button v-if="auth.hasPerm('customer:assign')" block @click="transferShow = true">转移</van-button>
        <van-button v-if="auth.hasPerm('customer:assign')" block @click="moveToSea">移入客户公海</van-button>
        <van-button v-if="auth.hasPerm('customer:delete')" block type="danger" plain @click="handleDelete">删除</van-button>
      </div>
    </van-action-sheet>

    <van-popup v-model:show="transferShow" position="bottom" round>
      <van-picker :columns="memberColumns" @confirm="transferOwner" @cancel="transferShow = false" />
    </van-popup>

    <van-popup v-model:show="relationShow" position="bottom" round>
      <div class="p-4">
        <div class="text-center font-medium mb-3">{{ relationEditingId ? '编辑客户关系' : '添加客户关系' }}</div>
        ...
        <van-picker :columns="relationColumns" @change="selectRelationCustomer" @confirm="selectRelationCustomer" />
    ...
    <van-popup v-model:show="collaboratorShow" position="bottom" round>
      <div class="p-4">
        <div class="text-center font-medium mb-3">{{ collaboratorEditingId ? '编辑协作设置' : '添加协作人' }}</div>
        <van-radio-group v-model="collaborationType" direction="horizontal" class="mb-3 justify-center">
        ...
        />
        <van-cell v-else title="协作人" :value="fieldRefs.memberMap.value.get(collaboratorUserId) ?? '-'" />
