...
function reset() {
  ...
  Object.assign(form, {
    ...
    recycleConditions: (pool?.recycleRule?.conditions ?? []).map((condition) => ({
      ...
      fixedRange: condition.operator === 'FIXED' ? condition.value.split(',').filter(Boolean) : [],
    })),
  })
}

async function save(continueCreate = false) {
  if (!form.name.trim()) return ElMessage.warning('请输入公海名称')
  if (!form.scopeIds.length) return ElMessage.warning('请选择公海成员')
  if (!form.managerIds.length) return ElMessage.warning('请选择公海管理员')
  ...
  try {
    ...
    ElMessage.success(props.pool ? '客户公海已更新' : '客户公海已添加')
    emit('saved')
...
<template>
  ...
  >
    <el-alert
      ...
      type="warning"
      :closable="false"
      title="修改公海规则后会立即影响当前公海成员、领取和自动回收规则。"
    />
    <el-form label-position="left" label-width="120px">
      <el-divider content-position="left">基本信息</el-divider>
      <el-form-item label="公海名称" required><el-input v-model="form.name" maxlength="255" /></el-form-item>
      <el-form-item label="公海管理员" required>
      ...
      <el-divider content-position="left">客户领取规则</el-divider>
      ...
      <el-divider content-position="left">客户回收规则</el-divider>
      ...
      <div class="mt-2 text-xs text-[var(--el-text-color-secondary)]">客户名称固定显示；这里仅控制当前客户公海页面字段可见性。</div>
    </el-form>
    <template #footer>
