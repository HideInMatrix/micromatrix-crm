...
async function load() {
  ...
  try {
    const [{ data: list }, { data: config }] = await Promise.all([
      dictionaryApi.list('OPPORTUNITY_FAIL_RS'),
...
async function toggle(value: boolean) {
  try {
    await dictionaryApi.toggle('OPPORTUNITY_FAIL_RS', value)
    enabled.value = value
...
async function add() {
  ...
  try {
    await dictionaryApi.add('OPPORTUNITY_FAIL_RS', name)
    draft.value = ''
    await load()
  } catch (error) {
    ElMessage.error(extractErrorMessage(error))
...
<template>
  <el-drawer v-model="visible" size="600px" destroy-on-close data-testid="opportunity-failure-reason-settings-drawer">
    <template #header>
      <div class="flex w-full items-center justify-between pr-4">
        <span class="font-medium">商机失败原因设置</span>
        <el-switch :model-value="enabled" :disabled="rows.length === 0" @change="toggle($event as boolean)" />
      </div>
    </template>
    <div class="mb-4 flex gap-2">
      <el-input v-model="draft" maxlength="255" placeholder="请输入失败原因" @keyup.enter="add" />
      <el-button type="primary" :disabled="!draft.trim() || rows.length >= 50" @click="add">添加</el-button>
    </div>
    <div class="mb-3 text-xs text-[var(--el-text-color-secondary)]">最多 50 条；开启后人工将商机推进到失败阶段必须选择原因，系统自动关闭使用保留原因 system。</div>
        ...
        <template #item="{ element: row }">
          <div class="mb-2 flex items-center gap-2 rounded border border-[var(--el-border-color)] p-3">
            ...
            <template v-else>
              ...
              <el-button link type="danger" :disabled="enabled && rows.length <= 1" @click="remove(row)">删除</el-button>
            </template>
          </div>
        </template>
      </draggable>
