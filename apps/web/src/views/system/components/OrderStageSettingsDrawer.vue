...
import {
  orderStageSettingsApi,
  type OrderStageAdvancedRowVO,
  type OrderStageConfigItemVO,
} from '@/api/system'

const visible = defineModel<boolean>({ required: true })
const loading = ref(false)
const saving = ref(false)
const rows = ref<OrderStageConfigItemVO[]>([])
const afootRollBack = ref(true)
const endRollBack = ref(false)
const circulationType = ref<'NORMAL' | 'ADVANCED'>('NORMAL')
const advancedConfigs = ref<OrderStageAdvancedRowVO[]>([])
...
async function load() {
  loading.value = true
  try {
    const { data } = await orderStageSettingsApi.get()
...
function openEdit(row: OrderStageConfigItemVO) {
...
async function save() {
  ...
  try {
    if (editingId.value) {
      await orderStageSettingsApi.update({ id: editingId.value, name: form.name.trim() })
    } else {
      const target = [...rows.value].reverse().find((row) => row.type === 'AFOOT')
      await orderStageSettingsApi.add({
        name: form.name.trim(),
        type: 'AFOOT',
        targetId: target?.id,
        dropPosition: 1,
...
async function remove(row: OrderStageConfigItemVO) {
  const ok = await ElMessageBox.confirm(`确定删除阶段「${row.name}」吗？`, '删除阶段', { type: 'warning' }).catch(() => false)
  if (!ok) return
  try {
    await orderStageSettingsApi.remove(row.id)
...
async function updateRollback() {
  try {
    await orderStageSettingsApi.updateRollback({
...
async function sort() {
  try {
    await orderStageSettingsApi.sort(rows.value.map((row) => row.id))
  } catch (error) {
    ElMessage.error(extractErrorMessage(error))
    await load()
  }
...
async function changeCirculationType(value: 'NORMAL' | 'ADVANCED') {
  try {
    await orderStageSettingsApi.switchCirculationType(value)
...
function setTransition(originId: string, targetId: string, enable: boolean) {
  ...
  if (!row) {
    row = { originId, moduleType: 'order', targets: [] }
...
async function saveAdvanced() {
  saving.value = true
  try {
    await orderStageSettingsApi.saveAdvancedConfig({
...
<template>
  <el-drawer v-model="visible" title="订单状态流设置" size="820px" destroy-on-close data-testid="order-stage-settings-drawer">
    <el-alert class="mb-4" type="info" :closable="false" title="最多 15 个订单阶段。进行中阶段可新增、改名、排序；已有合同数据的阶段禁止删除。" />
