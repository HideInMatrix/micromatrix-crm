...
function dynamicValues(row: QuoteVO) {
  ...
  return Object.fromEntries(
    dynamicFields.value.map((field) => [field.key, valueById.get(field.id)]),
  )
}
...
async function openDetail(id: string) {
  ...
  try {
    const [{ data }, { data: snapshot }] = await Promise.all([
      quoteApi.detail(id),
...
async function handleVoid(row: QuoteVO) {
  if (
    !(await ElMessageBox.confirm(`确认作废报价「${row.name}」吗？作废后不可恢复。`, '作废报价', {
      type: 'warning',
    }).catch(() => false))
  )
    return
...
async function handleRevoke(row: QuoteVO) {
  if (
    !(await ElMessageBox.confirm(`撤回报价「${row.name}」当前审批？`, '撤回审批').catch(
      () => false,
    ))
  )
    return
...
async function handleDelete(row: QuoteVO) {
  if (
    !(await ElMessageBox.confirm(
      `确认删除报价「${row.name}」吗？配置删除审批时会先提交审批。`,
      '删除报价',
      { type: 'warning' },
    ).catch(() => false))
  )
...
async function batchVoid() {
  if (
    !(await ElMessageBox.confirm(
      `确认作废选中的 ${selectedIds.value.length} 个报价吗？`,
      '批量作废',
      { type: 'warning' },
    ).catch(() => false))
  )
...
async function batchApprove() {
  ...
  try {
    const { data } = await quoteApi.batchApprove({
      ids: selectedIds.value,
      approvalStatus: batchApprovalStatus.value,
    })
...
async function exportPdf(row: QuoteVO) {
  try {
    const [{ data: snapshot }] = await Promise.all([
      quoteApi.snapshot(row.id),
      quoteApi.download(row.id),
    ])
    const snapshotProducts = Array.isArray(snapshot.products)
      ? (snapshot.products as Array<Record<string, unknown>>)
      : []
    ...
    const productHtml = snapshotProducts
      .map(
        (item) =>
          `<tr><td>${escapeHtml(item.productName ?? item.productId ?? item.product)}</td><td>${escapeHtml(item.priceName ?? item.priceId)}</td><td>${escapeHtml(item.productAmount)}</td><td>${escapeHtml(item.discount)}</td><td>${escapeHtml(item.tax)}</td><td>${escapeHtml(item.amount)}</td></tr>`,
      )
      .join('')
...
<template>
  <el-card shadow="never">
    ...
    >
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        <AdvancedFilter
          ...
          @apply="(conditions) => ((filters = conditions), (query.current = 1), loadData())"
        />
    ...
    <el-table
      ...
      stripe
      class="w-full"
      @selection-change="(selected: QuoteVO[]) => (selectedIds = selected.map((row) => row.id))"
    >
      ...
      >
        <template #default="{ row }">
          ...
          <el-button
            v-else-if="column.key === 'opportunityId'"
            link
            type="primary"
            @click="toOpportunity(asQuote(row))"
            >{{ row.opportunityName || row.opportunityId }}</el-button
          >
          <template v-else-if="column.key === 'amount'"
            >¥{{ Number(row.amount).toLocaleString('zh-CN') }}</template
          >
          <template v-else-if="column.key === 'untilTime'">{{
            new Date(row.untilTime).toLocaleDateString('zh-CN')
          }}</template>
          <template v-else>{{
            formatFieldValue(column, displayRow(asQuote(row)), {
      ...
      <el-table-column label="审批状态" width="120"
        ><template #default="{ row }"
          ><el-tag :type="approvalTagType(row.approvalStatus)" size="small">{{
            QUOTATION_APPROVAL_STATUS_LABELS[row.approvalStatus as QuoteVO['approvalStatus']]
          }}</el-tag></template
        ></el-table-column
      >
      <el-table-column label="状态" width="90"
        ><template #default="{ row }"
          ><el-tag :type="row.invalid ? 'info' : 'success'" size="small">{{
            row.invalid ? '已作废' : '正常'
          }}</el-tag></template
        ></el-table-column
      >
        ...
        <template #default="{ row }">
          <template v-if="!row.invalid">
            ...
            <el-button
              ...
              link
              type="success"
              @click="handleApproval(asQuote(row), 'APPROVED')"
              >通过</el-button
            >
            <el-button
              v-if="row.approvalStatus === 'APPROVING'"
              link
              type="danger"
              @click="handleApproval(asQuote(row), 'UNAPPROVED')"
              >驳回</el-button
            >
            <el-button
              v-if="row.approvalStatus === 'APPROVING' && row.createUser === auth.user?.id"
              link
              @click="handleRevoke(asQuote(row))"
              >撤回</el-button
            >
          ...
          <el-button
            v-if="row.approved && !row.invalid"
            link
            type="primary"
            @click="toContract(asQuote(row))"
            >创建合同</el-button
          >
          <el-button
            ...
            link
            type="danger"
            @click="handleDelete(asQuote(row))"
            >删除</el-button
          >
        </template>
      </el-table-column>
  ...
  >
    <el-form label-position="top">
      <el-form-item label="报价名称" required
        ><el-input v-model="quotationName" maxlength="255" show-word-limit
      /></el-form-item>
      <el-form-item label="商机" required>
        <el-select
          ...
          filterable
          remote
          :remote-method="loadOpportunities"
          class="w-full"
          placeholder="搜索并选择商机"
          ><el-option
      ...
      <el-form-item label="有效期至" required
        ><el-date-picker v-model="untilTime" type="date" value-format="x" class="!w-full"
      /></el-form-item>
      <DynamicForm
        ref="formRef"
        v-model="formModel"
      ...
      <el-form-item label="累计金额" required>
        ...
        <div class="text-xs text-[var(--el-text-color-secondary)] mt-1">
          当前 Cordys 源码未提供固定行金额公式，前端不硬编码推导公式。
        </div>
      </el-form-item>
    </el-form>
    ...
    <el-table :data="quotationProducts" border>
      <el-table-column label="产品" min-width="160"
        ><template #default="{ row }"
          ><el-select
            ...
            filterable
            class="w-full"
            @change="onProductChange(asProductEdit(row))"
            ><el-option
      ...
      ></el-table-column>
      <el-table-column label="价格表" min-width="150"
        ><template #default="{ row }"
          ><el-select
            v-model="row.priceId"
            clearable
            filterable
            class="w-full"
            @change="onPriceChange(asProductEdit(row))"
            ><el-option
              ...
              :value="item.id" /></el-select></template
      ></el-table-column>
      <el-table-column label="产品定价" width="125"
        ><template #default="{ row }"
          ><el-input-number
            v-model="row.productAmount"
            :min="0"
            :precision="2"
            :controls="false"
      ...
      <el-table-column label="折扣" width="110"
        ><template #default="{ row }"
          ><el-input-number
            ...
            class="!w-full" /></template
      ></el-table-column>
      <el-table-column label="税点" width="110"
        ><template #default="{ row }"
          ><el-input-number
            v-model="row.tax"
            :precision="2"
      ...
      <el-table-column width="60"
        ><template #default="{ $index }"
          ><el-button link type="danger" @click="quotationProducts.splice($index, 1)"
            >删</el-button
          ></template
        ></el-table-column
      >
    ...
    <template #footer
      ...
      ><el-button type="primary" :loading="formSaving" @click="saveQuote">保存</el-button></template
    >
  </el-drawer>
  ...
  <el-drawer v-model="detailVisible" :title="detail?.name || '报价详情'" size="760px">
      ...
      <template v-if="detail">
        <div class="flex items-center gap-2 mb-4">
          <el-tag :type="approvalTagType(detail.approvalStatus)">{{
            QUOTATION_APPROVAL_STATUS_LABELS[detail.approvalStatus]
          }}</el-tag
          ><el-tag :type="detail.invalid ? 'info' : 'success'">{{
            detail.invalid ? '已作废' : '正常'
          }}</el-tag>
        </div>
        <el-descriptions :column="2" border>
          <el-descriptions-item label="报价名称">{{ detail.name }}</el-descriptions-item>
          <el-descriptions-item label="商机"
            ><el-button link type="primary" @click="toOpportunity(detail)">{{
              detail.opportunityName
            }}</el-button></el-descriptions-item
          >
          <el-descriptions-item label="有效期至">{{
            new Date(detail.untilTime).toLocaleDateString('zh-CN')
          }}</el-descriptions-item>
          <el-descriptions-item label="累计金额"
            >¥{{ detail.amount.toLocaleString('zh-CN') }}</el-descriptions-item
          >
          <el-descriptions-item
            ...
            :label="field.label"
            >{{
              formatFieldValue(field, displayRow(detail), {
                memberMap: fieldRefs.memberMap.value,
                deptMap: fieldRefs.deptMap.value,
              })
            }}</el-descriptions-item
        ...
        <el-table :data="detail.products" border
          ><el-table-column prop="productName" label="产品" min-width="150" /><el-table-column
            prop="priceName"
            label="价格表"
            ...
            prop="discount"
            label="折扣"
            width="90" /><el-table-column prop="tax" label="税点" width="90" /><el-table-column
            prop="amount"
            label="金额"
            width="110"
        /></el-table>
    ...
    <template v-if="detail" #footer
      ><el-button @click="exportPdf(detail)">导出 PDF</el-button
      ><el-button
        v-if="detail.approved && !detail.invalid"
        type="primary"
        plain
        @click="toContract(detail)"
        >创建合同</el-button
      ><el-button
        v-if="!detail.invalid && detail.approvalStatus !== 'APPROVING'"
        type="primary"
  ...
  />
  <el-dialog v-model="batchApprovalVisible" title="批量审批" width="420px"
    ><el-form label-position="top"
      ><el-form-item label="审批结果" required
        ><el-radio-group v-model="batchApprovalStatus"
          ><el-radio value="APPROVED">通过</el-radio
          ><el-radio value="UNAPPROVED">驳回</el-radio></el-radio-group
        ></el-form-item
      ></el-form
    ><template #footer
      ><el-button @click="batchApprovalVisible = false">取消</el-button
      ><el-button type="primary" :loading="batchLoading" @click="batchApprove"
        >确认</el-button
      ></template
    ></el-dialog
  >
