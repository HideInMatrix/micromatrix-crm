...
import { callbackLark, callbackLarkMobile, callbackLarkOauth } from '@/api/auth'
...
onMounted(async () => {
  ...
  try {
    const callback = state.startsWith('lark-mobile.')
      ? callbackLarkMobile
      : state.startsWith('lark.')
        ? callbackLarkOauth
        : state.startsWith('qr-lark.')
          ? callbackLark
          : null
    if (!callback) throw new Error('飞书登录状态类型无效')
...
<template>
  <div class="h-full flex-center bg-[var(--el-bg-color-page)]">
    <el-card class="w-120" shadow="never">
      <el-result v-if="errorMessage" icon="error" title="飞书登录失败" :sub-title="errorMessage">
        <template #extra>
