...
import type { ContactVO, FieldVO, OpportunityVO } from '@micromatrix/shared'
...
function displayCustomField(field: FieldVO) {
  if (!detail.value) return '-'
  return formatFieldValue(field, detail.value as unknown as Record<string, unknown>, {
...
async function load() {
  ...
  loading.value = true
  try {
    const [{ data: opportunity }, { data: contactResult }, { data: fieldRows }] = await Promise.all(
      [
              ...
              <el-descriptions :column="3" border>
                ...
                <el-descriptions-item label="预计金额">
                  {{
                    detail.amount === null
                      ? '-'
                      : `¥${Number(detail.amount).toLocaleString('zh-CN')}`
                  }}
                </el-descriptions-item>
                <el-descriptions-item label="可能性">{{
                  detail.possible == null ? '-' : `${detail.possible}%`
                }}</el-descriptions-item>
                ...
                <el-descriptions-item label="最近跟进时间">{{
                  detail.followTime ? new Date(detail.followTime).toLocaleString() : '-'
                }}</el-descriptions-item>
              </el-descriptions>
            </el-card>
          ...
          <el-table v-else :data="contacts" border>
            ...
            <el-table-column label="当前关联" width="100">
              <template #default="{ row }"
                ><el-tag v-if="row.id === detail?.contactId" type="primary" size="small"
                  >当前</el-tag
                ></template
              >
            </el-table-column>
