    ...
    <el-form label-width="92px">
      <el-form-item label="计划对象" required>
        <div class="flex gap-2 w-full">
          <el-select v-model="form.targetType" :disabled="targetLocked" class="!w-32">
      ...
      <el-form-item v-if="form.targetType !== 'lead'" label="联系人">
        <el-select v-model="form.contactId" clearable filterable class="w-full" placeholder="可选">
      ...
      <el-form-item label="跟进方式">
        <el-select v-model="form.method" clearable class="w-full">
          <el-option v-for="item in ['电话', '拜访', '微信', '邮件', '会议', '其他']" :key="item" :label="item" :value="item" />
        </el-select>
      </el-form-item>
      <el-form-item label="负责人">
        <el-select v-model="form.ownerId" clearable filterable class="w-full" placeholder="默认当前用户">
          <el-option v-for="item in members" :key="item.id" :label="item.name" :value="item.id" />
        </el-select>
      </el-form-item>
      <el-form-item label="计划内容" required>
        <el-input v-model="form.content" type="textarea" :rows="4" maxlength="3000" show-word-limit />
      </el-form-item>
    </el-form>
    <template #footer>
