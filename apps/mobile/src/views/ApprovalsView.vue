...
function taskStatusLabel(status: string) {
  return status === 'APPROVED' ? '已同意' : status === 'REJECTED' ? '已驳回' : status === 'SKIPPED' ? '已跳过' : '待处理'
}
</script>
  ...
  <div class="min-h-full">
    ...
    <van-popup v-model:show="detailShow" position="bottom" round :style="{ height: '75%' }">
      <div v-if="current" class="p-4 flex flex-col h-full">
        ...
        <van-steps direction="vertical" :active="current.tasks.length" class="flex-1 overflow-y-auto">
          <van-step v-for="task in current.tasks" :key="task.id">
            <div class="text-sm">
              {{ task.nodeName }} · {{ task.approverName ?? '-' }} ·
              {{ taskStatusLabel(task.status) }}
            </div>
            <div v-if="task.comment" class="text-xs text-gray-500 mt-1">意见：{{ task.comment }}</div>
