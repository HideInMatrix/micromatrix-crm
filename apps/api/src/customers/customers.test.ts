...
test(
  'CustomersService production 路径使用 Prisma 保持 CRUD、团队与集团关系语义',
  { skip: !databaseUrl },
  async () => {
    ...
    try {
      const tenant = await createPrismaTestTenant(prismaClient, 'customers-p8')
      tenantId = tenant.id
      ...
      const collaborator = await createPrismaTestUser(prismaClient, {
        tenantId: tenant.id,
      ...
      const accessResult = async (customerId: string) => {
        const customer = await prismaClient.orm.public.Customer.where({
          id: customerId,
      ...
      assert.equal(detail.name, 'Prisma 客户集团')

      const updated = await service.update(user, parent.id, { name: 'Prisma 客户集团更新' })
      assert.equal(updated.name, 'Prisma 客户集团更新')
      assert.equal(
        (
      ...
      const remaining = await prismaClient.orm.public.Customer.where((row) =>
      ...
      if (tenantId) {
        ...
        if (customerIds.length) {
          ...
          await prismaClient.orm.public.CustomerRelation.where((row) =>
