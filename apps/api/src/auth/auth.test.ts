...
test(
  'AuthService 使用 Prisma 保持注册、登录、refresh、改密与 LoginLog 会话语义',
  { skip: !databaseUrl },
  async () => {
    ...
    const tenantName = `Prisma Auth ${suffix.slice(0, 8)}`
    ...
    try {
      ...
      const persistedUser = await prismaClient.orm.public.Users.where({ id: registered.user.id })
        .select('deptId', 'authVersion', 'defaultPwd')
        .first()
      assert.ok(persistedUser)
      const persistedRoleLink = await prismaClient.orm.public.UserRoles.where({
        tenantId,
      ...
      const persistedRole = await prismaClient.orm.public.Roles.where({
      ...
      const login = await service.login(
        ...
        { ip: '127.0.0.1', userAgent: 'prisma-auth-gate' },
      )
      ...
      const logs = await prismaClient.orm.public.LoginLogs.where({
        userId: registered.user.id,
        authType: 'PASSWORD',
      })
        ...
        .all()
      assert.equal(
        logs.some((item) => item.success),
        true,
      )
      assert.equal(
        logs.some((item) => !item.success && item.message === '邮箱或密码错误'),
        true,
      )
      ...
      const externalFailureLogs = await prismaClient.orm.public.LoginLogs.where({
        tenantId,
        userId: registered.user.id,
        authType: 'WECOM',
        success: false,
      ...
      await service.changePassword(registered.user.id, 'old-password-123', 'new-password-456')
      const changed = await prismaClient.orm.public.Users.where({ id: registered.user.id })
      ...
      if (tenantId) {
        ...
        await prismaClient.orm.public.Subscriptions.where({ tenantId }).deleteAll()
