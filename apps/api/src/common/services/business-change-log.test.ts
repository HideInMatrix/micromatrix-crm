...
test(
  'BusinessChangeLog 使用 Prisma 同事务写主日志与 JSONB Blob，并由原生 ORM readback 验证',
  { skip: !databaseUrl },
  async () => {
    ...
    try {
      const tenant = await createPrismaTestTenant(prismaClient, 'p8-change-log')
      tenantId = tenant.id
      ...
      const service = new BusinessChangeLogService({ client: prismaClient } as PrismaService)
      ...
      const row = await prismaClient.orm.public.OperationLogs.where({
        tenantId: tenant.id,
        targetId: `customer-${suffix}`,
      })
      ...
      const blob = await prismaClient.orm.public.OperationLogBlobs.where({ operationLogId: row.id })
        .select('detail')
        .first()
      ...
      const unchangedRows = await prismaClient.orm.public.OperationLogs.where({
        tenantId: tenant.id,
        targetId: `unchanged-${suffix}`,
      })
        .select('id')
