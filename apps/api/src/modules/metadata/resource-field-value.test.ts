...
test(
  'Metadata Prisma raw filter 覆盖 14 类静态资源表并保持组织隔离与字段语义',
  { skip: !databaseUrl },
  async () => {
    ...
    try {
      ...
      const actor = actorId
      const target = await prisma.orm.public.Customer.select('id').create({
        id: createLegacyId32(),
        name: `Prisma Metadata ${randomUUID()}`,
      ...
      const lowScore = await prisma.orm.public.Customer.select('id').create({
        id: createLegacyId32(),
        name: `Prisma Metadata low ${randomUUID()}`,
        organizationId: organizationId,
        createTime: timestamp,
        updateTime: timestamp,
        createUser: actor,
      ...
      const otherTenant = await prisma.orm.public.Customer.select('id').create({
        id: createLegacyId32(),
        name: `Prisma Metadata other ${randomUUID()}`,
      ...
      await prisma.orm.public.CustomerField.createAll(
        [
      ...
      await prisma.orm.public.CustomerFieldBlob.createAll(
        [
          {
            resourceId: target.id,
      ...
      if (customerIds.length) {
        await prisma.orm.public.Customer.where((row) => row.id.in(customerIds)).deleteAll()
      }
      await testDb.close()
    }
  },
