...
test('Bidding production 路径完整使用 Prisma：配置、订阅、抓取、分页、手工导入与转线索', async (t) => {
  ...
  const tenant = await createPrismaTestTenant(prismaClient, 'p8-bidding')
  ...
  const service = new BiddingService(prisma, { save: async () => undefined } as never, provider)

  try {
    assert.deepEqual(await service.saveSource(user, 'demo', true, { apiKey: 'prisma-test' }), {
      name: 'Prisma Demo Source',
    })
    ...
    assert.deepEqual(await service.addKeyword(user, ' Prisma '), { name: 'Prisma' })
    let keywords = await service.listKeywords(tenant.id)
    assert.equal(keywords.length, 1)
    assert.equal(keywords[0]?.keyword, 'Prisma')
    ...
    const infos = await prismaClient.orm.public.BiddingInfos.where({ tenantId: tenant.id })
    ...
    assert.equal(
      infos[0]?.publishedAt ? instantToISOString(infos[0].publishedAt) : null,
      publishedAt.toISOString(),
    )

    const refreshedSource = await prismaClient.orm.public.BiddingSources.where({
      tenantId: tenant.id,
    }).first()
    assert.ok(refreshedSource)
    assert.ok(refreshedSource.lastFetchAt)
    assert.deepEqual(refreshedSource.credentials, { apiKey: 'prisma-test' })
    ...
    assert.equal(filtered.items[0]?.title, 'Prisma 标讯 A')
    assert.equal(filtered.items[0]?.budget, 123456.78)

    const lead = await service.convertToLead(user, manual.id)
    const storedLead = await prismaClient.orm.public.Clue.where({
      id: lead.id,
    }).first()
    assert.ok(storedLead)
    assert.equal(storedLead.organizationId, tenant.id)
    assert.equal(storedLead.owner, owner.id)
    const converted = await prismaClient.orm.public.BiddingInfos.where({ id: manual.id }).first()
    assert.ok(converted)
    assert.equal(converted.convertedLeadId, lead.id)
  } finally {
    await prismaClient.orm.public.Clue.where({ organizationId: tenant.id }).deleteAll()
    await prismaClient.orm.public.BiddingInfos.where({ tenantId: tenant.id }).deleteAll()
    await prismaClient.orm.public.BiddingKeywordSubs.where({ tenantId: tenant.id }).deleteAll()
    await prismaClient.orm.public.BiddingSources.where({ tenantId: tenant.id }).deleteAll()
