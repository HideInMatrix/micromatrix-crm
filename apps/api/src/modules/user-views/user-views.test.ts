...
test(
  'UserViews 使用 Prisma 保持 ownership、条件替换、4096 排序与唯一约束语义',
  { skip: !databaseUrl },
  async () => {
    ...
    try {
      const tenant = await createPrismaTestTenant(prismaClient, 'p8-user-views')
      tenantId = tenant.id
      ...
      const service = new UserViewsService({ client: prismaClient } as PrismaService)
      ...
      assert.equal(
        (
          await prismaClient.orm.public.SysUserViewCondition.where({
      ...
      assert.equal(
        await prismaClient.orm.public.SysUserView.where({
          id: first.id,
        })
          .select('id')
          .first(),
      ...
      if (tenantId) {
        ...
        const viewIds = await prismaClient.orm.public.SysUserView.where({ organizationId })
          .select('id')
          .all()
        if (viewIds.length) {
          await prismaClient.orm.public.SysUserViewCondition.where((row) =>
            row.sysUserViewId.in(viewIds.map((item) => item.id)),
          ).deleteAll()
        }
        await prismaClient.orm.public.SysUserView.where({ organizationId }).deleteAll()
