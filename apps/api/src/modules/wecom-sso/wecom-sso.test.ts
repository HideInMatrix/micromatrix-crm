...
test('WeCom SSO 使用 Prisma 保持 discovery、身份绑定与 OAuth state 持久化语义', async (t) => {
  const databaseUrl = process.env['DATABASE_URL']
  if (!databaseUrl) return t.skip('DATABASE_URL 未配置')
  const config = {
    getOrThrow: () => databaseUrl,
    get: (key: string) =>
      key === 'WECOM_OAUTH_REDIRECT_URI'
  ...
  const tenant = await createPrismaTestTenant(prismaClient, 'p8-wecom')
  ...
  try {
    ...
    assert.equal(discovery.agentId, integration.agentId)
