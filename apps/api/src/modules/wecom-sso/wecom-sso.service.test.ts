...
test('企微 OAuth state 绑定浏览器、只消费一次并复用本地账号', async () => {
  ...
  const integration = {
    id: 'integration-a', tenantId: tenant.id, provider: 'WECOM', corpId: 'ww-a',
    agentId: '1000001', lastTestSucceeded: true, syncEnabled: true,
  }
  ...
  const mapping: any = {
    id: 'mapping-a', tenantId: tenant.id, provider: 'WECOM', externalId: 'ZhangSan',
    externalKey: 'zhangsan', userId: mappedUser.id, active: true, lastSeenBatchId: null,
    createdAt: prisma8TimestampFromDate(new Date()), updatedAt: prisma8TimestampFromDate(new Date()),
  }
  ...
  const config = {
    get: (key: string) => key === 'WECOM_DEFAULT_TENANT_SLUG' ? tenant.slug : key === 'WEB_PUBLIC_URL' ? 'http://localhost:5173' : key === 'NODE_ENV' ? 'test' : undefined,
  } as unknown as ConfigService
  const integrations = {
    getActivePlatform: async () => ({ syncResource: 'WECOM', sync: true }),
    getWeComRuntimeContext: async () => ({ integration, credentials: { corpId: integration.corpId, agentId: integration.agentId, appSecret: 'secret' } }),
  } as unknown as EnterpriseIntegrationsService
  const client = {
    exchangeLoginCode: async () => ({ userId: 'ZhangSan', externalKey: 'zhangsan' }),
    exchangeOAuthLoginCode: async () => ({ userId: 'ZhangSan', externalKey: 'zhangsan', email: 'zhangsan@work.example.com', phone: '13800000001', avatarUrl: 'https://example.com/avatar.png', gender: true }),
  ...
  const service = new WeComSsoService(prisma8, config, integrations, client, auth)

  const started = await service.start({ returnPath: '/customers' })
  assert.equal(started.value.corpId, integration.corpId); assert.ok(started.value.state.startsWith('qr-wecom.'))
  ...
  assert.ok(workbench.value.state.startsWith('wecom.')); assert.equal(oauthState.flow, 'WECOM')
  const workbenchUrl = new URL(workbench.value.authorizationUrl.replace('#wechat_redirect', ''))
  assert.equal(workbenchUrl.origin + workbenchUrl.pathname, 'https://open.weixin.qq.com/connect/oauth2/authorize')
  assert.equal(workbenchUrl.searchParams.get('scope'), 'snsapi_base')
  await assert.rejects(() => service.callback({ code: 'wrong-flow-code', state: workbench.value.state }, workbench.browserNonce, { ip: '127.0.0.1', userAgent: 'node-test' }), UnauthorizedException)
  const workbenchResult = await service.callbackWorkbench({ code: 'workbench-code', state: workbench.value.state }, workbench.browserNonce, { ip: '127.0.0.1', userAgent: 'wxwork node-test' })
  assert.equal(workbenchResult.returnPath, '/home'); assert.equal(loginCalls, 2)
  assert.deepEqual(updatedProfile, { phone: '13800000001', gender: true }); assert.equal(updatedAvatar, 'https://example.com/avatar.png')

  const entry = await service.startWorkbenchEntry({ target: 'http://localhost:5173/teacher?source=wecom#overview' }, 'http://localhost:5173')
  ...
  await assert.rejects(() => service.startWorkbenchEntry({ target: 'https://attacker.example/steal' }, 'http://localhost:5173'), BadRequestException)
})
