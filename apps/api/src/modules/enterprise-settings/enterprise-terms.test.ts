...
test(
  'EnterpriseTerms 使用 Prisma 保持分类统计、关键词查询与 discovery 采纳事务',
  { skip: !databaseUrl },
  async () => {
    ...
    try {
      const tenant = await createPrismaTestTenant(prismaClient, 'p8-terms')
      tenantId = tenant.id
      const user = { id: `user-${suffix}`, tenantId: tenant.id } as AuthUser
      const service = new EnterpriseTermsService({ client: prismaClient } as PrismaService)
      ...
      const discovery = await prismaClient.orm.public.EnterpriseTermDiscoveries.select('id').create(
        {
          tenantId: tenant.id,
          discovered: 'ARR',
          source: 'AI',
      ...
      const storedDiscovery = await prismaClient.orm.public.EnterpriseTermDiscoveries.where({
        id: discovery.id,
      }).first()
      ...
      assert.equal(
        (
          await prismaClient.orm.public.EnterpriseTerms.where({ tenantId: tenant.id })
            .select('id')
            .all()
        ).length,
        2,
      ...
      if (tenantId) {
        await prismaClient.orm.public.EnterpriseTermDiscoveries.where({ tenantId }).deleteAll()
        await prismaClient.orm.public.EnterpriseTerms.where({ tenantId }).deleteAll()
        await prismaClient.orm.public.EnterpriseTermCategories.where({ tenantId }).deleteAll()
