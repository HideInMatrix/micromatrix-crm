...
test(
  ...
  async () => {
    ...
    try {
      ...
      await prisma8Client.orm.public.OpportunityQuotationField.where({
        id: quotationFieldId,
      }).delete()
      ...
      assert.equal(
        (await prisma8Client.orm.public.Product.where({ organizationId: org }).select('id').all())
          .length,
        0,
      )
    } finally {
      ...
      if (quotationIds.length) {
        ...
        await prisma8Client.orm.public.OpportunityQuotationField.where((row) =>
          row.resourceId.in(ids),
        ).deleteAll()
        await prisma8Client.orm.public.OpportunityQuotation.where((row) =>
          row.id.in(ids),
        ).deleteAll()
      }
