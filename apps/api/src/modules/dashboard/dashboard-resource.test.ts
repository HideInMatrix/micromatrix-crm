...
test(
  'DashboardResource 使用 Prisma 保持 CRUD、收藏、分页与移动事务语义',
  { skip: !databaseUrl },
  async () => {
    ...
    try {
      const tenant = await createPrismaTestTenant(prismaClient, 'p8-dashboard-resource')
      tenantId = tenant.id
      const user = await createPrismaTestUser(prismaClient, {
        tenantId: tenant.id,
        name: '看板资源管理员',
      })
      const moduleA = await prismaClient.orm.public.DashboardModule.select('id', 'name').create({
        id: createLegacyId32(),
      ...
      const moduleB = await prismaClient.orm.public.DashboardModule.select('id', 'name').create({
        id: createLegacyId32(),
        organizationId: tenant.id,
        name: '销售目录',
        createTime: 1n,
      ...
      const service = new DashboardResourceService(prismaService, access)
      ...
      assert.equal(
        (
          await prismaClient.orm.public.Dashboard.where({ id: first.id })
            .select('description')
            .first()
        )?.description,
        '第一张看板',
      ...
      const storedFirst = await prismaClient.orm.public.Dashboard.where({
        id: first.id,
      })
      ...
      const movedSecond = await prismaClient.orm.public.Dashboard.where({
        id: second.id,
      })
      ...
      const moduleBRows = await prismaClient.orm.public.Dashboard.where({
        organizationId: tenant.id,
        dashboardModuleId: moduleB.id,
      })
      ...
      assert.equal(
        (await prismaClient.orm.public.Dashboard.where({ id: second.id }).select('id').all())
          .length,
        0,
      )
    } finally {
