...
test(
  'BusinessTitle 使用 Prisma 保持高级筛选、审批状态、配置与发票引用保护语义',
  { skip: !databaseUrl },
  async () => {
    ...
    try {
      const tenant = await createPrismaTestTenant(prismaClient, 'p8-business-title')
      tenantId = tenant.id
      ...
      const config = await prismaClient.orm.public.BusinessTitleConfig.select('id').create({
        id: createLegacyId32(),
      ...
      const invoice = await prismaClient.orm.public.ContractInvoice.select('id').create({
        id: createLegacyId32(),
      ...
      await prismaClient.orm.public.ContractInvoice.where({ id: invoice.id }).delete()
      assert.deepEqual(await service.remove(user, alpha.id), { id: alpha.id, name: alpha.name })
      const fixtureReadback = await prismaClient.orm.public.BusinessTitle.where({
        id: beta.id,
      }).first()
      ...
      if (tenantId) {
        ...
        await prismaClient.orm.public.ContractInvoice.where({ organizationId }).deleteAll()
        ...
        await prismaClient.orm.public.BusinessTitleConfig.where({ organizationId }).deleteAll()
        await prismaClient.orm.public.BusinessTitle.where({ organizationId: tenantId }).deleteAll()
