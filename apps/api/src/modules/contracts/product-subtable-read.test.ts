...
test(
  '产品子表 batch reader 使用 Prisma 保持租户隔离、normal/blob 合并与名称装配',
  { skip: !databaseUrl },
  async () => {
    ...
    try {
      ...
      const actor = actorId
      const product = await prismaClient.orm.public.Product.select('id').create({
        id: id(),
        name: 'Prisma 产品',
        status: 'ENABLED',
        pos: 1n,
      ...
      const price = await prismaClient.orm.public.ProductPrice.select('id').create({
        id: id(),
        name: '标准价格表',
        status: 'ENABLED',
        pos: 1n,
      ...
      const foreignContract = await prismaClient.orm.public.Contract.select('id').create({
        id: id(),
        name: '合同 B',
      ...
      const order = await prismaClient.orm.public.SalesOrder.select('id').create({
        id: id(),
      ...
      await prismaClient.orm.public.ContractField.createAll(
        [
          {
            id: id(),
      ...
      const priceRow = id()
      await prismaClient.orm.public.ProductPriceField.createAll(
        [
          {
            id: id(),
            resourceId: price.id,
      ...
      await prismaClient.orm.public.ProductPriceFieldBlob.create({
        id: id(),
        resourceId: price.id,
        refSubId: priceFields[0]!.id,
        rowId: priceRow,
      ...
      const orderRow = id()
      await prismaClient.orm.public.SalesOrderField.createAll(
        [
          {
            id: id(),
            resourceId: order.id,
      ...
      await prismaClient.orm.public.SalesOrderFieldBlob.create({
        id: id(),
        resourceId: order.id,
        refSubId: orderFields[0]!.id,
        rowId: orderRow,
      ...
      const quoteRow = id()
      await prismaClient.orm.public.OpportunityQuotationField.createAll(
        [
          {
            id: id(),
      ...
      await prismaClient.orm.public.OpportunityQuotationFieldBlob.create({
        id: id(),
        resourceId: quotation.id,
        refSubId: quoteFields[0]!.id,
        rowId: quoteRow,
      ...
      const contractService = new ContractFieldsService(prisma, forms(contractFields))
      const priceService = new ProductPriceFieldsService(prisma, forms(priceFields))
      const orderService = new OrderFieldsService(prisma, forms(orderFields))
      const quoteService = new QuotationFieldsService(prisma, forms(quoteFields))
      ...
      await prismaClient.orm.public.SalesOrder.where({ organizationId: org }).deleteAll()
      await prismaClient.orm.public.Contract.where((row) =>
        row.organizationId.in([org, foreignOrg]),
      ).deleteAll()
      await prismaClient.orm.public.Customer.where((row) =>
        row.organizationId.in([org, foreignOrg]),
      ).deleteAll()
