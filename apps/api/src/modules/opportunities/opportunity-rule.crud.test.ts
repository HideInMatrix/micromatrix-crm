...
test('OpportunityRule 使用 Prisma 保持 CRUD、分页、scope 名称与阶段校验语义', async (t) => {
  ...
  const tenant = await createPrismaTestTenant(prismaClient, 'p8-opp-rule')
  ...
  const service = new OpportunityRuleService(prisma)
  ...
  try {
    ...
    assert.equal(updated?.name, 'Prisma 更新规则')
    ...
    const persisted = await prismaClient.orm.public.OpportunityRule.where({
      id: created.id,
    }).first()
    assert.ok(persisted)
    assert.equal(persisted.enable, false)
    assert.equal(persisted.name, 'Prisma 更新规则')

    await service.remove(tenant.id, created.id)
    assert.equal(
      (
        await prismaClient.orm.public.OpportunityRule.where({
          id: created.id,
        })
          .select('id')
          .all()
    ...
    await prismaClient.orm.public.OpportunityRule.where({ organizationId }).deleteAll()
