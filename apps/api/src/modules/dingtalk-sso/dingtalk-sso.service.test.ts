...
test('钉钉 OAuth state 绑定浏览器、只消费一次并按 userid 映射本地账号', async () => {
  ...
  const config = {
    get: (key: string) =>
      key === 'DINGTALK_DEFAULT_TENANT_SLUG'
        ? tenant.slug
        : key === 'WEB_PUBLIC_URL'
          ? 'http://localhost:5173'
          : key === 'NODE_ENV'
  ...
  const service = new DingTalkSsoService(prisma, config, integrations, client, auth)
  ...
  assert.equal(oauthState.flow, 'DINGTALK')
  assert.equal(
    (
      await service.callbackWorkbench(
