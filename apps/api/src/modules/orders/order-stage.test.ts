...
test(
  'OrderStage 使用 Prisma 保持默认阶段、groupBy、强事务排序与高级流转语义',
  { skip: !databaseUrl },
  async () => {
    ...
    try {
      const tenant = await createPrismaTestTenant(prismaClient, 'p8-order-stage')
      tenantId = tenant.id
      ...
      const service = new OrderStageService({ client: prismaClient } as PrismaService)
      ...
      await prismaClient.orm.public.SalesOrder.where({ id: order.id }).delete()
      assert.deepEqual(await service.remove(user, addedId), { id: addedId, name: '补充订单阶段' })
      const remaining = await prismaClient.orm.public.SalesOrderStageConfig.where({
        organizationId,
      })
      ...
      if (tenantId) {
        ...
        await prismaClient.orm.public.SalesOrder.where({ organizationId }).deleteAll()
        await prismaClient.orm.public.StageAdvancedConfig.where({ organizationId }).deleteAll()
        await prismaClient.orm.public.SalesOrderStageConfig.where({ organizationId }).deleteAll()
