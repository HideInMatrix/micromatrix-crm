...
export interface HarnessComment {
  ...
  updatedAt: ReturnType<typeof instantFromDate>
}
...
export function createFollowCommentPrismaHarness(options: HarnessOptions) {
  const calls: string[] = []
  ...
  const resources = [
    {
      ...
      updatedAt: instantFromISOString('2026-09-06T03:00:00.000Z'),
    },
  ]
  ...
  const commentHooks = {
    onCreate: (data: Record<string, unknown>) => {
      ...
      return {
        ...
        createTime: instantFromDate(now),
        updateTime: data['updateTime'] ?? instantFromDate(now),
