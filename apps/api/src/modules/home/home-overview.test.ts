...
test('HomeOverview 使用 Prisma 保持 owner scope、阶段聚合、排行、趋势与转化语义', async (t) => {
  ...
  const tenant = await createPrismaTestTenant(prismaClient, 'p8-home-overview')
  ...
  const other = await createPrismaTestUser(prismaClient, {
    tenantId: tenant.id,
  ...
  try {
    await prismaClient.orm.public.OpportunityStageConfig.createAll([
      {
        ...stageBase,
        id: stageAfoot,
        name: '推进',
    ...
    await prismaClient.orm.public.Customer.createAll([
      {
        id: createLegacyId32(),
        name: '客户一',
        owner: ownerId,
    ...
    const service = new HomeOverviewService(prisma, dataScope)
