...
test(
  'CluePoolConfig 使用 Prisma 保持 noPick、部门 scope 管理员与用户名称装配',
  { skip: !databaseUrl },
  async () => {
    ...
    try {
      const tenant = await createPrismaTestTenant(prismaClient, 'p8-clue-pool')
      tenantId = tenant.id
      ...
      const pool = await prismaClient.orm.public.CluePool.select(
        'id',
        'name',
        'scopeId',
        'organizationId',
      ...
      await prismaClient.orm.public.Clue.create({
        id: createLegacyId32(),
        name: 'Pool Lead',
        owner: null,
        stage: 'NEW',
      ...
      if (tenantId) {
        ...
        await prismaClient.orm.public.CluePool.where({ organizationId }).deleteAll()
