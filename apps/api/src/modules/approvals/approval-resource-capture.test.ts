...
test('ApprovalResourceCapture 使用 Prisma 保持 Contract JSON 快照与租户隔离', async (t) => {
  ...
  const tenant = await createPrismaTestTenant(prismaClient, 'p8-capture')
  const otherTenant = await createPrismaTestTenant(prismaClient, 'p8-capture-other')
  const actorId = id32()
  ...
  try {
    ...
    const service = new ApprovalResourceCaptureService(prisma)
    ...
    assert.equal(captured.contract.name, 'Prisma 快照合同')
    ...
    await prismaClient.orm.public.Contract.where({ organizationId: tenant.id }).deleteAll()
    await prismaClient.orm.public.Customer.where({ organizationId: tenant.id }).deleteAll()
    await prismaClient.orm.public.Tenants.where((row) =>
