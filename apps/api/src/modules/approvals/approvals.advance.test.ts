...
test(
  '审批 advance 使用 Prisma transaction 原子写入自动通过与下一节点任务',
  { skip: !databaseUrl },
  async () => {
    ...
    try {
      const tenant = await createPrismaTestTenant(prismaClient, 'p8-approval-advance')
      await prismaClient.orm.public.Users.createAll([
        {
          id: submitterId,
          tenantId: tenant.id,
          name: 'Submitter',
      ...
      const persistedInstance = await prismaClient.orm.public.ApprovalInstances.where({
        id: instance.id,
      }).first()
      ...
      const records = await prismaClient.orm.public.ApprovalRecords.where({
        tenantId: tenant.id,
        instanceId: instance.id,
        nodeId: firstNodeId,
      }).all()
      ...
      const tenant = await prismaClient.orm.public.Tenants.where({
