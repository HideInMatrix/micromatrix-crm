...
test('ApprovalFlowConfig 使用 Prisma 保持流程图版本、排序与编号计数语义', async (t) => {
  ...
  const tenant = await createPrismaTestTenant(prismaClient, 'p8-flow')
  const approver = await createPrismaTestUser(prismaClient, {
    tenantId: tenant.id,
  ...
  try {
    ...
    const deleted = await prismaClient.orm.public.ApprovalFlows.where({ id: first.id })
      .select('deletedAt')
      .first()
    ...
    const versions = await prismaClient.orm.public.ApprovalFlowVersions.where({
      flowId: first.id,
    })
      .select('id')
      .all()
    assert.equal(versions.length, 2)
    const nodes = await prismaClient.orm.public.ApprovalNodes.where((row) =>
    ...
    if (nodes.length) {
      await prismaClient.orm.public.ApprovalNodeApprovers.where((row) =>
        row.nodeId.in(nodes.map((item) => item.id)),
      ).deleteAll()
      await prismaClient.orm.public.ApprovalNodes.where((row) =>
        row.id.in(nodes.map((item) => item.id)),
      ).deleteAll()
    }
