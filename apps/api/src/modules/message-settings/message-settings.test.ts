...
test(
  'MessageSettings 使用 Prisma 持久化 JSON config、批量渠道设置与企业微信 gate',
  { skip: !databaseUrl },
  async () => {
    ...
    try {
      const tenant = await createPrismaTestTenant(prismaClient, 'p8-msg')
      tenantId = tenant.id
      await prismaClient.orm.public.Tenants.where({ id: tenant.id }).update({
        enterpriseSyncResource: 'WECOM',
        updatedAt: nowInstant(),
      })
      await prismaClient.orm.public.EnterpriseIntegrations.create({
        tenantId: tenant.id,
        provider: 'WECOM',
        corpId: `corp-${suffix}`,
