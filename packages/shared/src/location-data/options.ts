import { provinceAndCityData, regionData } from './region-data.js'
import type { LocationOption } from './types.js'

function mapTree(
  source: LocationOption[],
  mapper: (item: LocationOption, path: LocationOption[], level: number) => LocationOption,
  path: LocationOption[] = [],
): LocationOption[] {
  return source.map((sourceItem) => {
    const item: LocationOption = {
      ...sourceItem,
      children: sourceItem.children
        ? mapTree(sourceItem.children, mapper, [...path, sourceItem])
        : undefined,
    }
    return mapper(item, path, path.length)
  })
}
...
export const COUNTRIES_TREE: LocationOption[] = [
  {
    label: '阿鲁巴',
    value: 'ABW',
  },
