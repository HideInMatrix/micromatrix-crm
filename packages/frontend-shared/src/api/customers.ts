...
export async function listCustomers(
  ...
  const response = await http.post<CordysPager<CustomerVO>>(
    ...
    poolMode ? { ...customerPageBody(params), poolId: params.poolId } : customerPageBody(params),
  )
  return {
    ...response,
    data: {
...
export function batchTransferCustomers(ids: string[], owner: string) {
  return http.post<{ success: number; fail: number; failedIds: string[] }>(
    '/account/batch/transfer',
    { ids, owner },
  )
}
...
export async function listCustomerOptions(
  keyword?: string,
): Promise<AxiosResponse<CustomerOptionVO[]>> {
