...
async function clueListRequest(
  params: LeadListParams,
): Promise<AxiosResponse<PaginatedResult<LeadVO>>> {
...
export const resourcePoolApi = {
  ...
  customerSettingsPage: async (params?: {
    current?: number
    pageSize?: number
    keyword?: string
  }) => {
  ...
  remove: (id: string, module: 'lead' | 'customer') =>
    module === 'lead'
      ? http.get(`/lead-pool/delete/${id}`)
      : http.get(`/account-pool/delete/${id}`),
...
export const leadApi = {
  ...
  exportAll: (params: LeadListParams, data: ExportCreatePayload, poolId?: string) =>
    http.post(poolId ? '/pool/lead/export-all' : '/lead/export', {
      ...cluePageBody(params),
      ...data,
      ...(poolId ? { poolId } : {}),
    }),
  ...
  claim: (id: string, poolId: string) => http.post('/pool/lead/pick', { clueId: id, poolId }),
  transfer: (id: string, owner: string) => http.post('/lead/batch/transfer', { ids: [id], owner }),
  poolAssign: (id: string, assignUserId: string) =>
    http.post('/pool/lead/assign', { clueId: id, assignUserId }),
  ownerHistory: (id: string) => http.get<OwnerHistoryVO[]>(`/lead/owner/history/list/${id}`),
...
export const opportunityApi = {
  ...
  create: async (data: Record<string, unknown>) => {
    ...
    if (
      ...
      data.stageId &&
      data.stageId !== response.data.stageId
    ) {
  ...
  createStage: async (data: { name: string; probability: number }) => {
    ...
    const created = [...stages.data]
      .reverse()
      .find((stage) => !stage.system && stage.name === data.name)
...
export const contactApi = {
  ...
  checkOpportunity: (id: string) => http.get<boolean>(`/account/contact/opportunity/check/${id}`),
...
export const customerExtraApi = {
  ...
  ownerHistory: (id: string) => http.get<OwnerHistoryVO[]>(`/account/owner/history/list/${id}`),
  teamList: (id: string) => http.get<TeamMemberVO[]>(`/account/collaboration/list/${id}`),
  teamAdd: (
    id: string,
    userId: string,
    role?: string,
